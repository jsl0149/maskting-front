...
const LoginAuth = () => {
	...
	if (role === 'guest') {
		if (sort === 'true') {
			...
			window.location.href = `${REACT_APP_CLIENT_HOST}wait`;
		} else {
			if (provider) {
				// 가입이 필요한 경우
				window.location.href = `${REACT_APP_CLIENT_HOST}/signup/basicInfo`;
			} else {
				// 심사 거절된 경우 -> 대기 + 심사 거절 화면
				window.location.href = `${REACT_APP_CLIENT_HOST}/wait/fail`;
			}
		}
	} else {
		// 모든 가입 완료한 상태 -> 피드 화면
		window.location.href = `${REACT_APP_CLIENT_HOST}/home`;
	}
};
